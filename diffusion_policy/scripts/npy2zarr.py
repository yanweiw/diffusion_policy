...
def main(datasource='logged_plans_03'):
    ...
    for file in tqdm(data_files):
        ...
        obs = np.concatenate(ee_gripper_cube, axis=1) # ee pos, gripper, red, green, blue, yellow, cyan, magenta
        action_list.append(action)
        obs_list.append(obs)
        state_list.append(plan)
    ...
    data.create_dataset('obs', data=np.concatenate(obs_list, axis=0), dtype='float32')
